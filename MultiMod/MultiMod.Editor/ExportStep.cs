...
namespace MultiMod.Editor
{
    ...
    public class GetContent : ExportStep
    {
        ...
        internal override void Execute(ExportSettings settings, ExportData data)
        {
            data.asmDefs = GetAssets("t:AssemblyDefinitionAsset");
        }
    ...
    public class ImportScripts : ExportStep
    {
        ...
        internal override void Execute(ExportSettings settings, ExportData data)
        {
            ...
            foreach (var asmDef in data.asmDefs)
            {
                var text = File.ReadAllText(asmDef.assetPath);
                var asmDefObj = JsonUtility.FromJson<AsmDef>(text);

                string scriptAssemblyPath = Path.Combine(assemblyDirectory, $"{asmDefObj.name}.dll");
    ...
    public class Export : ExportStep
    {
        ...
        internal override void Execute(ExportSettings settings, ExportData data)
        {
            ...
            BuildAssetBundles(ModPlatform.Windows);
